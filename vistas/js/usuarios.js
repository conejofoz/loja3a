...
function validarContactenos(){
    ...
    if(email == ""){
        ...
        var expresion = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/;
        if(!expresion.test(email)){
            $("#emailContactenos").before('<h6 class="alert alert-danger">Por favor escriba correctamente el email</h6');
            return false;
        }
    }
    ...
    if(mensaje == ""){
        ...
        if(!expresion.test(mensaje)){
            $("#mensajeContactenos").before('<h6 class="alert alert-danger">Por favor escriba el mensaje sin caracteres especiales</h6');
            return false;
        }
    }
